    ...
    commands: {
      ...
      cat(...params) {
        ...
        if (!Alpine.store('db').files[params[0]])
          return this.output({ type: 'error', message: 'File does not exist!' })
        ...
        fileContent.innerText = Alpine.store('db').files[params[0]]
      ...
      service(...params) {
        if (params.length === 0) {
          ...
          return document.querySelector('#commandOutput').appendChild(container)
